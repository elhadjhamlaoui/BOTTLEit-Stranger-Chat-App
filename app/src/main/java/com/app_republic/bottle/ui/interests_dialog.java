...
public class interests_dialog extends DialogFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        loadInterests();
        adapter = new Adapter(getActivity(), interests);
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                progressBar.setVisibility(View.VISIBLE);
                  FirebaseDatabase.getInstance().getReference().child("user").child(StaticConfig.UID).child("interests").setValue(user.interests).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
    ...
    class Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
            ...
            if (user.interests.contains(interest.name))
            ...
            relativeLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (checkBox.isChecked()){
                        ...
                        b = false;
                        user.interests = user.interests.replace("/"+interests.get(getAdapterPosition()).name,"");
                        interests.get(getAdapterPosition()).checked = b;
                        } else {
                        if (ServiceUtils.getCount(user.interests) < 5){
                            checkBox.setChecked(true);
                            b = true;
                            user.interests = user.interests + "/" + interests.get(getAdapterPosition()).name;
                            interests.get(getAdapterPosition()).checked = b;
                        }
                    }
    ...
    private void loadInterests() {
        try {
            BufferedReader reader = new BufferedReader(
                    new InputStreamReader(getActivity().getAssets().open("interests.txt")));

            String line;
            while ((line = reader.readLine()) != null) {
                interests.add(new Interest(line, false));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
